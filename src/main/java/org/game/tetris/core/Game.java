...
import org.game.tetris.GameLauncher;
...
public class Game {
    ...
    private final AudioClip s_game_over = create("sound/gameover.wav", 0.5, false);
    private final AudioClip s_clear = create("sound/clear.wav", 0.5, false);
    private final AudioClip s_line = create("sound/line.wav", 0.5, false);
    private final AudioClip s_rotate = create("sound/rotate.wav", 0.5, false);
    private final AudioClip s_fall = create("sound/fall.wav", 0.5, false);
    private final AudioClip s_slow_hit = create("sound/slow-hit.wav", 0.5, false);
    ...
    private void check_for_tetris() {
        ...
        for (int i = HEIGHT - 2; i >= 1; i--) {
            ...
            var val = isFilled(i);
            if (val.equals(Line.FILLED) || val.equals(Line.HALF_FILLED)) {
                if (val.equals(Line.FILLED)) {
                    c++;
                } else {
                    if (c > 0) {
                        if (c == 1)
    ...
    private void draw() {
        ...
        int h = display.getSize().getRows() / 2 - (HEIGHT) / 2;
        int w = display.getSize().getColumns() / 2 - (WIDTH * 3 + 8) / 2;
        s.append("\n".repeat(h)).append(message()).append("\n").append("\n");
        for (int i = 0; i < box.length; i++) {
            ...
            switch (i) {
                ...
                case 12 -> s.append(" Hold key : ").
                        append(hold_key ? "on" : "off");
            }
            s.append('\n');
        }
        System.out.println(s.toString().indent(w));
    }
    ...
    private void is_rotatable_rotate(int x0, int x1, int x3, int y0, int y1, int y3) {
        if (isValidPoints(x0, x1, x3, y0, y1, y3) &&
                box[y0][x0] == null && box[y1][x1] == null && box[y3][x3] == null) {
            tetrominoes[0].x = x0;
            tetrominoes[1].x = x1;
            tetrominoes[3].x = x3;
            tetrominoes[0].y = y0;
    ...
    private boolean isValidPoints(int... c) {
        for (int i = 0; i <= 2; i++) {
            if (c[i] <= 0 || c[i] >= WIDTH - 1)
                return false;
        }
        for (int i = 3; i <= 5; i++) {
            if (c[i] <= 0 || c[i] >= HEIGHT - 1)
                return false;
        }
        return true;
    }
    ...
    private Line isFilled(int i) {
        ...
        if (m == WIDTH - 2)
            return Line.FILLED;
        return Line.HALF_FILLED;
    }

    public static AudioClip create(String name, double volume, boolean isRepeatable) {
        var url = Objects.requireNonNull(GameLauncher.class.getResource(name)).toString();
